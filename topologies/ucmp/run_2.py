...
def run(start_time):
    ...
    for ToR_pair in set_all_paths:
        for n in range(1, set_all_paths[ToR_pair]['optimal']):
            route.MAX_HOP = n
            ...
            for m in range(n, 0, -1):
                kept_pairs = [pair for pair in zip(paths, slices) if len(pair[1]) == m]
                if len(kept_pairs) > 0:
                    set_all_paths[ToR_pair][n] = []
                    for pair in kept_pairs:
                        set_all_paths[ToR_pair][n].append({'path': pair[0], 'slice': pair[1]})
                    break

    write_all_raw_paths(file_path + 'fully_reconf_all_paths/', file_name, set_all_paths, mode='all')
